...
class BrandAction extends CommonAction {
    ...
    public function index() {
        ...
        $showPage = $page->show();
        $list = $M->where($wheres)->field("`id`,`title`,`status`,`paixu`,`published`,`cid`,`aid`")->order("paixu asc,`status` ASC,`published` DESC")->limit($page->firstRow, $page->listRows)->select();
		...
		$category = M('brandcategory')->field("`cid`,`name`")->select();
    ...
    public function category() {
        if (IS_POST) {
            echo json_encode(D("Brand")->category());
        } else {
        	$this->assign("list", D("Brand")->category());
            $this->display();
        }
    }
    ...
    public function add() {
        if (IS_POST) {
            $this->checkToken();
            echo json_encode(D("Brand")->addBrand());
        } else {
			$info['status'] = 1;
			$info['reshow'] = 0;
			$info['fenxiang'] = 0;
			...
			$paixu = M("Brand")->limit(0,1)->order('paixu desc')->find();
    ...
    public function checkBrandTitle() {
        $M = M("Brand");
	...
	public function zongpai(){
		...
		foreach ($arrid as $k=>$v){
			M("Brand")->save(array('id'=>$v,'paixu'=>$arrpaixu[$k]));
			}
	...
	public function submit() {
		...
		if(count(explode(',',$id))>1){
		  ...
		  $list=M("Brand")->where($where)->delete();   
		  if($list!==false) {     
  ...
  public function guanlian(){
		...
		if(IS_POST){
			...
			if($act=='add'){
				 if ($M->add($wheres)) {
					  echo json_encode(array("status" => 1, "info" => "添加成功" ,'url' => U('Brand/guanlian?cid='.$cid.'')));
				  } else {
				 ...
				 if ($M->save($wheres)) {
					  echo json_encode(array("status" => 1, "info" => "修改成功" ,'url' => U('Brand/guanlian?cid='.$cid.'')));
				  } else {
	...
	public function guansubmit() {
		...
		if(count(explode(',',$id))>1){
		  ...
		  $list=M("brandlove")->where($where)->delete();   
		  if($list!==false) {     
		    echo json_encode(array('status' => 1, 'info' => "成功删除{$list}条")); 
			}else{    
			echo json_encode(array('status' => 0, 'info' => "删除失败" ));  
