...
class CommonAction extends Action {
    ...
    public function _initialize() {
        ...
        if (empty($systemConfig['TOKEN']['admin_marked'])) {
            $systemConfig['TOKEN']['admin_marked'] = "zhouyun";
            $systemConfig['TOKEN']['admin_timeout'] = 3600;
            $systemConfig['TOKEN']['member_marked'] = "http://www.duzui.org";
...
//                     echo L('_VALID_ACCESS_');
    ...
    private function show_sub_menu() {
		...
		/*---获取数据库自动添加信息类---*/
		if($big=="Informaction"){
		$listco = M('Informaction')->select();
		$lo = $cache['Informaction'];
		foreach ($listco as $k=>$v){
			$lo['editbox?id='.$v['id']] = $v['title'];//
			//[执行页面?id=数据id] = 标题
			}
		$cache['Informaction'] = $lo;
		unset($lo);
		}
		/*--------完毕--------*/
        $sub_menu = array();
