...
// 流体布局与 Ajax 自动翻页实现
$(function() {
 	$('.masonry-brick').css({display: 'none'});
	$("#masonrybox .masonry-brick").css({ opacity: 1 });
	// 流体布局  
    var $container = $('#masonrybox');
    $container.imagesLoaded(function(){//图片加载完再显示imgbox
	  $('.masonry-brick').show();
    });
	
	// 流体布局与 Ajax 自动翻页实现
	var request,nextHref = $("#next_page a").attr("href");
	//给浏览器窗口绑定scroll时间
	$(window).bind("scroll", function(){
		//判断窗口的滚动条是否已经接近页面底部
		//alert(($(document).scrollTop() + $(window).height())+"|||"+($(document).height() - $("#footer").height()));
        if( $(document).scrollTop() + $(window).height() > $(document).height() - $("#footer").height() ) {
			//判断下一页连接是否为空
			if( nextHref != undefined ) {
				//如果上次的ajax分页操作存在，或者状态！=4时中断终止上一个没有完成Ajax请求request.abort()
				if(request && request.readyState != 4 ) {
					request.abort();
				}
				//var htmlobj=$.ajax({url:$("#next_page a").attr("href"),async:false});
				//alert(htmlobj.responseText);

				// Ajax 翻页
				request = $.ajax( {
					url: $("#next_page a").attr("href"),
					//type: "POST",//纯html页面就去掉type: "POST",动态获取的时候加上
					dataType:"html",
					beforeSend: function(){
						$("#page_loading").show("slow");
					},
					success: function(data) {//data返回html
						result = $(data).find("#masonrybox .masonry-brick");
						nextHref = $(data).find("#next_page a").attr("href");
						$("#next_page a").attr("href", nextHref);//更新下一页连接
						$("#masonrybox").append(result);//往$("#masonrybox")中加入新一页的内容
						
						$newElems=result.css({opacity:0});//把新的内容设置为透明
						$newElems = result;
